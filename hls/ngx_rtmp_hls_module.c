    ...
    ngx_int_t discont)
{
    ...
    ngx_sprintf(ctx->stream.data + ctx->stream.len, "%uL.bmp%Z", id);
...
ngx_rtmp_hls_publish(ngx_rtmp_session_t *s, ngx_rtmp_publish_t *v)
{
    ...
    *ngx_cpymem(ctx->name.data, v->name, ctx->name.len) = 0;
    ...
    len = hacf->path.len + 1 + ctx->name.len + sizeof(".doc");
    if (hacf->nested) {
        len += sizeof("/index") - 1;
    }
    ...
    if (hacf->variant) {
        ...
        for (n = 0; n < hacf->variant->nelts; n++, var++) {
                ...
                == 0)
            {
                ...
                ctx->var_playlist.len = len - var->suffix.len + sizeof(".doc")
                                        - 1;
                ...
                pp = ngx_cpymem(ctx->var_playlist.data, ctx->playlist.data,
                               len - var->suffix.len);
                pp = ngx_cpymem(pp, ".doc", sizeof(".doc") - 1);
                *pp = 0;
    ...
    if (hacf->nested) {
        p = ngx_cpymem(p, "/index.doc", sizeof("/index.doc") - 1);
    } else {
        p = ngx_cpymem(p, ".doc", sizeof(".doc") - 1);
    }
